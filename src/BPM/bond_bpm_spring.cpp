...
// adding a helper function for computing the damping
...
void BondBPMSpring::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if ((e > ecrit[type]) && break_flag) { // deleted fabs(e) to make e to only allow failure in tension
      bondlist[n][2] = 0;
    ...
    // modifying the smoothing to make it one sided as bonds can't fail in compression now
    if (smooth_flag) {
      smooth = (r - r0) / (r0 * ecrit[type]);
      double sgn = sign(smooth);
      double smooth_pow8 = smooth * smooth;
      smooth_pow8 *= smooth_pow8;  // ^4
      smooth_pow8 *= smooth_pow8;  // ^8
      smooth = 1.0 - 0.5 * (1.0 + sgn) * smooth_pow8;
